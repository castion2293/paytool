...
abstract class ECPay_Verification
{
    ...
    {
        $arPayMentExtend = array_merge(
            array_keys($this->arPayMentExtend),
            ($InvoiceMark == '') ? array() : $this->arInvoice
        );
    ...
    public function check_invoiceString($arExtend = array())
    {
        ...
        if (!array_key_exists('InvoiceItems', $arExtend)) {
            ...
            if ($arExtend['TaxType'] == ECPay_TaxType::Mix) {
                if (in_array(ECPay_TaxType::Dutiable, $tmpItemTaxType) and in_array(
                    ECPay_TaxType::Free,
                    $tmpItemTaxType
                )) {
                    // Do nothing
            ...
            if ((count($tmpItemName) + count($tmpItemCount) + count($tmpItemWord) + count($tmpItemPrice) + count(
                $tmpItemTaxType
            )) == (count($tmpItemName) * 5)) {
